...
use crate::internal::{updater, keys};
use crate::swap::error::ErrorKind;
use crate::types::NodeClient;
use crate::{Error, AcctPathMapping, OutputCommitMapping, InitTxArgs, OutputStatus};
use crate::{
    wallet_lock, WalletInst, WalletLCProvider,
};
use grin_keychain::{ExtKeychainPath, Identifier};
use std::sync::Arc;
use crate::api_impl::{owner, foreign};
use grin_core::libtx::{tx_fee, aggsig};
use crate::Context;
use std::collections::HashMap;
use uuid::Uuid;
use grin_util::secp::pedersen::Commitment;
use grin_util::secp;
use grin_util::secp::{PublicKey, Message};
...
impl IntegrityContext {
    ...
    pub fn calc_kernel_excess( &self, secp: &secp::Secp256k1, message: &Message ) -> Result<(Commitment, Signature), Error> {
        ...
        let final_sig = aggsig::add_signatures(secp, vec![&signature0, &signature1], &pub_nonce_sum)?;
...
) -> Result< (Option<AcctPathMapping>, Vec<OutputCommitMapping>, u64, Vec<(IntegrityContext, bool, u64)> ), Error>
    where
        ...
        K: Keychain + 'a,
{
    ...
    let integrity_account = w.acct_path_iter().filter( |a| u32::from(ExtKeychainPath::from_identifier(&a.path).path[0]) == INTEGRITY_ACCOUNT_ID ).next();
    ...
    let log_entry = updater::retrieve_txs( &mut **w, keychain_mask,
                                           None,
                                           None,
    ...
    for log_entry in log_entry {
        ...
        if log_entry.tx_slate_id.is_none() || height < tip_height - INTEGRITY_FEE_VALID_BLOCKS || log_entry.is_cancelled() {
            continue;
        }

        tx_uuid.insert(log_entry.tx_slate_id.unwrap(), (log_entry.confirmed, height) );
    }
    ...
    for (uuid, (confirmed, height)) in tx_uuid {
        ...
        integrity_tx.push((integrity_context, confirmed, height+INTEGRITY_FEE_VALID_BLOCKS));
    }
        ...
        K: Keychain + 'a,
{
    ...
    let (account, outputs, tip_height, mut transactions ) = get_integral_balance(wallet_inst.clone(), keychain_mask)?;
    ...
    if outputs.iter().find(|o| o.output.status == OutputStatus::Unconfirmed).is_some() {
        return Ok(results)
    }
    ...
    for i in 0..fee.len() {
        ...
        { // Let's run some test while we are in debug mode. Building full test is hard. We don't have self transactions tests
                ...
                let (excess, signature) = integrity_context.calc_kernel_excess(keychain.secp(), &kernel.msg_to_sign()?)?;
        ...
        results[i] = (Some(integrity_context), false, tip_height+INTEGRITY_FEE_VALID_BLOCKS);
...
pub fn withdraw_integral_balance<'a, L, C, K>(
    ...
    where
        ...
        K: Keychain + 'a,
{
    let (_account, outputs, _tip_height, _transactions ) = get_integral_balance(wallet_inst.clone(), keychain_mask)?;
